...
export default function Home() {
  ...
  return (
    <main className="flex min-h-screen flex-col bg-background-light dark:bg-background-dark font-display text-slate-800 dark:text-white relative">
      ...
      {/* Top Navigation Bar */}
      <header className="flex-none px-6 py-4 flex flex-wrap items-center justify-between w-full max-w-7xl mx-auto z-10 gap-y-4">

        {/* Left: Title */}
        <div className="flex items-center gap-3 order-1">
          <div className="w-10 h-10 md:w-12 md:h-12 bg-white dark:bg-surface-dark rounded-2xl flex items-center justify-center shadow-3d border-2 border-slate-100 dark:border-slate-700">
            <span className="material-icons-round text-primary text-2xl md:text-3xl">school</span>
          </div>
          <div>
            <h1 className="font-extrabold text-lg md:text-xl tracking-tight text-slate-800 dark:text-white leading-none">Sidang 24 Feb</h1>
            <p className="text-xs md:text-sm font-medium text-slate-500 dark:text-slate-400">Skripsi</p>
          </div>
        </div>

        {/* Center: Chunky Progress Bar */}
        <div className="flex flex-col items-center w-full md:w-1/3 order-3 md:order-2">
          <div className="flex justify-between w-full text-xs font-bold text-slate-400 mb-2 px-1 uppercase tracking-wide">
            <span>Progress</span>
            <span>{answeredCount} / {questions.length}</span>
          </div>
          <div className="h-4 md:h-5 w-full bg-slate-200 dark:bg-slate-700 rounded-full p-1 border border-slate-300 dark:border-slate-600 relative overflow-hidden">
        ...
        <div className="flex items-center gap-4 order-2 md:order-3">
          <div className="flex items-center gap-4 bg-white dark:bg-surface-dark px-3 py-1.5 md:px-4 md:py-2 rounded-full border-2 border-slate-100 dark:border-slate-700 shadow-sm">
            <div className="flex items-center gap-2">
              <span className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-accent-yellow"></span>
              <span className="text-xs md:text-sm font-semibold text-slate-500 dark:text-slate-400">Time: <span className="text-slate-800 dark:text-slate-200">{formatTime(elapsedTime)}</span></span>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs md:text-sm font-semibold text-slate-500 dark:text-slate-400">Left: <span className="text-slate-800 dark:text-slate-200">{questions.length - answeredCount}</span></span>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="flex-grow flex flex-col items-center justify-center px-4 py-6 relative w-full max-w-7xl mx-auto gap-8 md:gap-12">
        ...
        <div className="w-full max-w-3xl flex flex-col md:flex-row items-center justify-center gap-4 md:gap-6 px-4 pb-8 md:pb-0">
          ...
          <div className="flex gap-3 md:gap-4 w-full md:w-auto order-1">
            <button
              onClick={handleNext}
              className="flex-1 md:flex-none py-3 md:py-4 px-6 md:px-8 bg-white dark:bg-surface-dark text-accent-red font-bold rounded-xl md:rounded-2xl shadow-3d-red border-b-[4px] md:border-b-[6px] border-red-200 active:shadow-none active:translate-y-1.5 active:border-b-0 transition-all duration-150 flex items-center justify-center gap-2 md:gap-3 min-w-[120px] md:min-w-[140px] hover:bg-red-50 dark:hover:bg-red-900/20 text-sm md:text-base"
            >
              <span className="material-icons-round text-lg md:text-2xl">close</span>
              <span>Hard</span>
            </button>
            <button
              onClick={handleAnswered}
              className="flex-1 md:flex-none py-3 md:py-4 px-6 md:px-8 bg-primary text-slate-900 font-extrabold rounded-xl md:rounded-2xl shadow-3d-primary border-b-[4px] md:border-b-[6px] border-[#1e9e49] active:shadow-none active:translate-y-1.5 active:border-b-0 transition-all duration-150 flex items-center justify-center gap-2 md:gap-3 min-w-[140px] md:min-w-[160px] hover:brightness-105 text-sm md:text-base"
            >
              <span className="material-icons-round text-lg md:text-2xl">check</span>
              <span>I Knew It!</span>
            </button>
          </div>
